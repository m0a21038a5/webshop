	...
	public List<ShopLocation> shoplocationGetRepo() {
	  ...
	  public void addViewLog(String username,int product_id) {
		  ...
		  if(result == 0) {
			  ...
			  jdbcTemplate.update(insert,username,product_id,pro.getTitle(),pro.getAuthor(),pro.getRelease_day(),pro.getPrice(),pro.getGenre(),pro.getImgLink());
		  }else if(result > 0){
	...
	//--検索履歴--
	  public void saveSearchLog(SearchLog searchlog) {
		  ...
		  if(searchlog.getId() != 0) {
			  ...
			  result = jdbcTemplate.queryForObject(count, Integer.class,searchlog.getId(),searchlog.getUsername(),searchlog.getPlace());
			  
			  if(result > 0) {
				  String update = "UPDATE searchlog SET date = ? WHERE id = ? and username = ? and place = ?";
				  jdbcTemplate.update(update,LocalDateTime.now(),searchlog.getId(),searchlog.getUsername(),searchlog.getPlace());
			  }
		  }
		  ...
		  if(searchlog.getTitle() != null) {
			  ...
			  result = jdbcTemplate.queryForObject(count, Integer.class,searchlog.getTitle(),searchlog.getUsername(),searchlog.getPlace());
			  
			  if(result > 0) {
				  String update = "UPDATE searchlog SET date = ? WHERE title = ? and username = ? and place = ?";
				  jdbcTemplate.update(update,LocalDateTime.now(),searchlog.getTitle(),searchlog.getUsername(),searchlog.getPlace());
			  }
		  }
		  ...
		  if(searchlog.getAuthor() != null) {
			  ...
			  result = jdbcTemplate.queryForObject(count, Integer.class,searchlog.getAuthor(),searchlog.getUsername(),searchlog.getPlace());
			  
			  if(result > 0) {
				  String update = "UPDATE searchlog SET date = ? WHERE author = ? and username = ? and place = ?";
				  jdbcTemplate.update(update,LocalDateTime.now(),searchlog.getAuthor(),searchlog.getUsername(),searchlog.getPlace());
			  }
		  }
		  ...
		  if(searchlog.getGenre() != null) {
			  ...
			  result = jdbcTemplate.queryForObject(count, Integer.class,searchlog.getGenre(),searchlog.getUsername(),searchlog.getPlace());
			  
			  if(result > 0) {
				  String update = "UPDATE searchlog SET date = ? WHERE genre = ? and username = ? and place = ?";
				  jdbcTemplate.update(update,LocalDateTime.now(),searchlog.getGenre(),searchlog.getUsername(),searchlog.getPlace());
			  }
		  }
